...
bool hasNone(){
  std::pair <int,int> initBr=std::make_pair(-1,-1);
  return explored.count(initBr)>0;
}
...
static void inline penalty_multiply(double x,double y,uint64_t instID){
  DB("<<ENTERING __pen from instID = "<<instID <<
     "\n  with __d = "<<__d<<",     x, y = "<< x <<","<< y<<
     " and __r = "<<__r
     );
  __d++;

  __r= __r* (x-y)*(x-y)  + 1e-12;
   
  // __r*=  log10( (x-y)*(x-y)  + 1e-5+1 ) ;
  DB (">>LEAVING __pen with __r = "<<__r<<"\n");
  return;
}

static void inline penalty_min ( double x, double y , uint64_t instID){
  DB("<<ENTERING __pen from instID = "<<instID <<
     "\n  with __d = "<<__d<<",     x, y = "<< x <<","<< y<<
     " and __r = "<<__r
     );
#ifdef DEBUG
  std::cerr<<"coveredInstIDs = ";
  int i=0;
  for(auto inst:coveredInstIDs){
    std::cerr<<(i==0? "" : ",")<<inst;
    i++;
  }
  std::cerr<<std::endl;
#endif
  __d++;
  if (coveredInstIDs.count(instID)){
    DB (">>LEAVING __pen with __r = "<<__r<<"\n"
        <<"       because "<<instID <<" is already covered");
    return;
  }
  double dist=distance(x,y);

  if (dist<=config::TOL_PUSHSTAGED) {
    stagedInstIDs.insert (instID);
    //    __r=0;
  }//else
  __r= (__r<= dist? __r: dist);
...
extern "C" void pushStaged(){
  ...
  DB("Pushing happens: size(staged), size(covered) = "<<stagedInstIDs.size()<<", "<< coveredInstIDs.size());
  coveredInstIDs.insert(stagedInstIDs.begin(), stagedInstIDs.end());
  stagedInstIDs.clear();
  /*
    for (std::set<uint64_t>::iterator it=coveredInstIDs.begin(); it!=coveredInstIDs.end(); ++it)
...
extern "C" void removeSingleSidedExploredBranch(){
  ...
  assert (explored_afterRemoval.size()<=explored.size());
